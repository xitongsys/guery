...
func (self *Executor) SetInstructionDistinctLocal(instruction *pb.Instruction) (err error) {
	var enode eplan.EPlanDistinctLocalNode
...
func (self *Executor) RunDistinctLocal() (err error) {
	fname := fmt.Sprintf("executor_%v_distinctlocal_%v_cpu.pprof", self.Name, time.Now().Format("20060102150405"))
	f, _ := os.Create(fname)
	...
	enode := self.EPlanNode.(*eplan.EPlanDistinctLocalNode)
	//read md
